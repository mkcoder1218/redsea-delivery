...
const App: React.FC = () => {
  ...
  const handleCompleteDelivery = async () => {
    ...
    try {
      ...
      if (distanceMeters > 100) {
        throw new Error(`Out of Range: You are ${distanceMeters}m from the destination. Please move within 100m to finalize dropoff.`);
      }
  ...
  return (
    <div className="h-screen w-screen flex flex-col md:flex-row bg-slate-50 overflow-hidden relative">
      {/* Sidebar Task List */}
      <aside className={`fixed inset-y-0 left-0 w-full md:w-[400px] md:relative h-full bg-white border-r border-slate-200 flex flex-col z-[2000] shadow-xl transition-all duration-500 ease-in-out 
      ...
      `}>
        <header className="p-6 border-b border-slate-100 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => setIsDrawerOpen(false)} className="md:hidden p-2 text-slate-400 hover:text-slate-600 transition-colors bg-slate-50 rounded-lg border border-slate-100">
        ...
        <div className="p-6 space-y-4 bg-slate-50/50">
          ...
          <div className="grid grid-cols-3 gap-2">
            ...
            <div className="relative">
              <span className="absolute top-2 left-3 text-[8px] font-black text-slate-400 uppercase">Rad</span>
              <input type="number" step="any" value={radius} onChange={e => setRadius(Number(e.target.value))} className="w-full pt-5 pb-2 px-3 bg-white border border-slate-200 rounded-xl text-xs font-bold outline-none" />
            </div>
          </div>
      ...
      {selectedOrder && (
        <div className={`fixed inset-y-0 left-0 w-full md:w-[400px] bg-slate-900 text-white z-[2100] shadow-2xl transition-all duration-700 ease-out-expo flex flex-col ${orderState === 'delivered' ? 'translate-x-[-100%]' : 'translate-x-0'}`}>
