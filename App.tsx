...
const App: React.FC = () => {
  ...
  const [navMode, setNavMode] = useState<"integrated" | "external" | null>(
    null,
  );
  ...
  const [notificationsEnabled, setNotificationsEnabled] = useState(() => {
    return localStorage.getItem("redseamart_notifications") === "true";
  });
  ...
  const handleSearch = async (
    ...
    try {
      const searchLat = overrideLat ?? lat;
      const searchLng = overrideLng ?? lng;
      const response = await searchByCoordinates(
        searchLat,
        searchLng,
        radius,
        token,
  ...
  const handleCompleteDelivery = async () => {
    ...
    try {
      ...
      // PROXIMITY CHECK: Relaxed to 100 meters to account for GPS drift
      if (distanceMeters > 100) {
        throw new Error(
          `Out of Range: You are ${distanceMeters}m from the destination. Please move within 100m to finalize dropoff.`,
        );
      }
    ...
    if ("geolocation" in navigator) {
      navigator.geolocation.getCurrentPosition(
        (pos) => {
          ...
          setLat(newLat);
          setLng(newLng);
          handleSearch(undefined, undefined, newLat, newLng);
        },
        () => {
  ...
  useEffect(() => {
    const hasPreference = localStorage.getItem(
      "redseamart_notifications_preference_set",
    );
    if (!hasPreference && isAuthenticated) {
  ...
  const handleNotificationChoice = async (choice: boolean) => {
    ...
    localStorage.setItem("redseamart_notifications_preference_set", "true");
    localStorage.setItem("redseamart_notifications", String(choice));
    ...
    if (choice) {
      try {
        ...
        if (!fcmToken) {
          throw new Error(
            "Failed to get notification token. Please check your browser settings.",
          );
        }

        const authToken = localStorage.getItem("redseamart_token");

        // Subscribe to the "delivery" topic with the FCM token
        await subscribeToTopic("delivery", fcmToken, authToken);

        console.log("Successfully subscribed to delivery notifications");
      } catch (e: any) {
        console.error("Failed to subscribe", e);
        setError(e.message || "Failed to enable notifications");
        // Revert the notification enabled state on error
        setNotificationsEnabled(false);
        localStorage.setItem("redseamart_notifications", "false");
      }
    }
  };

  useEffect(() => {
    if (isAuthenticated) detectLocation();
  }, []);
  ...
  return (
    <div className="h-screen w-screen flex flex-col md:flex-row bg-slate-50 overflow-hidden relative">
      {/* Sidebar Task List */}
      <aside
        className={`fixed inset-y-0 left-0 w-full md:w-[400px] md:relative h-full bg-white border-r border-slate-200 flex flex-col z-[2000] shadow-xl transition-all duration-500 ease-in-out 
        ${orderState !== "idle" ? "md:-translate-x-full md:opacity-0 md:pointer-events-none" : ""}
      ...
      >
        <header className="p-6 border-b border-slate-100 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            {/* Mobile Toggle Button (Close Drawer) */}
            <button
              onClick={() => setIsDrawerOpen(false)}
              className="md:hidden p-2 text-slate-400 hover:text-slate-600 transition-colors bg-slate-50 rounded-lg border border-slate-100"
            >
              <svg
                className="w-5 h-5"
                fill="none"
            ...
            <button
              ...
              className="p-2 bg-slate-50 hover:bg-slate-100 rounded-lg text-slate-600 border border-slate-100 transition-colors"
            >
        ...
        <div className="p-6 space-y-4 bg-slate-50/50">
          <button
            ...
            className="w-full flex items-center justify-center gap-2 py-3 bg-white hover:bg-slate-50 rounded-xl text-[10px] font-black text-slate-600 border border-slate-200 shadow-sm transition-all uppercase tracking-widest"
          >
            <ICONS.Location className="w-4 h-4 text-emerald-500" />{" "}
            {t.detectLocation}
          </button>
          ...
          <div className="flex items-center justify-between px-3 py-2 bg-white rounded-xl border border-slate-200">
            ...
            <div
              className={`w-8 h-4 rounded-full relative transition-colors duration-300 ${notificationsEnabled ? "bg-emerald-500" : "bg-slate-300"}`}
            >
              <div
                className={`absolute top-0.5 bottom-0.5 w-3 h-3 bg-white rounded-full shadow-sm transition-all duration-300 ${notificationsEnabled ? "left-[calc(100%-14px)]" : "left-0.5"}`}
              />
            </div>
          </div>

          <div className="grid grid-cols-1 gap-2">
            {/* Lat and Lng inputs hidden as per requirement 
            ...
            */}
            <div className="relative">
              <span className="absolute top-2 left-3 text-[8px] font-black text-slate-400 uppercase z-10">
                Rad
              </span>
              <select
                value={radius}
                onChange={(e) => setRadius(Number(e.target.value))}
                className="w-full pt-5 pb-2 px-3 bg-white border border-slate-200 rounded-xl text-xs font-bold outline-none appearance-none relative z-0"
              >
                {Array.from({ length: 16 }, (_, i) => (i + 1) * 5).map((r) => (
          ...
          <button
            ...
            disabled={isLoading}
            className="w-full py-4 bg-slate-900 hover:bg-black text-white font-black rounded-2xl shadow-xl shadow-slate-900/20 transition-all flex items-center justify-center gap-3"
          >
            {isLoading ? (
      ...
      {selectedOrder && (
        <div
          className={`fixed inset-y-0 left-0 w-full md:w-[400px] bg-slate-900 text-white z-[2100] shadow-2xl transition-all duration-700 ease-out-expo flex flex-col ${orderState === "delivered" ? "translate-x-[-100%]" : "translate-x-0"}`}
        >
            ...
            <div className="flex justify-between items-start mb-10">
              <div className="animate-in fade-in slide-in-from-left-4 duration-500">
                <div className="inline-flex items-center gap-2 px-3 py-1 bg-emerald-500/10 border border-emerald-500/20 rounded-full mb-4">
                  <div
                    className={`w-1.5 h-1.5 rounded-full animate-pulse ${orderState === "shipping" ? "bg-blue-400" : "bg-emerald-500"}`}
                  />
                  <span
                    className={`text-[9px] font-black uppercase tracking-widest ${orderState === "shipping" ? "text-blue-400" : "text-emerald-400"}`}
                  >
                    {orderState === "idle"
                      ? "Pickup Confirmation"
                      : "In Transit"}
                ...
                <div className="p-6 bg-white/5 rounded-[32px] border border-white/10 animate-in zoom-in-95 duration-500">
                  ...
                  <div className="space-y-3">
                    <button
                      ...
                      className="w-full py-4 bg-white/5 hover:bg-white/10 border border-white/10 rounded-2xl flex flex-col items-center gap-0.5 transition-all"
                    >
                    ...
                    <button
                      onClick={() => selectNavigation("external")}
                      className="w-full py-4 bg-blue-600 hover:bg-blue-500 rounded-2xl flex flex-col items-center gap-0.5 transition-all"
                    >
                      <span className="text-xs font-black uppercase">
                        Google Maps
                      </span>
                ...
                <>
                  ...
                  <div className="bg-white/5 rounded-3xl overflow-hidden border border-white/5">
                    <div className="p-4 border-b border-white/5 bg-white/2 flex justify-between items-center">
                      ...
                      {navMode && (
                        <button
                          onClick={() => setNavMode(null)}
                          className="text-[8px] font-black text-blue-400 uppercase bg-blue-500/10 px-2 py-0.5 rounded border border-blue-500/20 hover:bg-blue-500/20 transition-colors"
                        >
                          Change Nav
                        </button>
                      )}
            ...
            <div className="mt-auto space-y-4">
                ...
                <div className="p-4 bg-rose-500/10 border border-rose-500/20 rounded-2xl animate-in fade-in slide-in-from-bottom-2 duration-300">
                  <div className="flex gap-3">
                    ...
                    <div className="flex flex-col">
                      ...
                      <button
                        onClick={() => setOrderPanelError("")}
                        className="text-[8px] font-black text-rose-500 uppercase mt-2 w-fit"
                      >
                        Dismiss
                      </button>
                    </div>
              ...
              {(orderState === "idle" ||
                ...
                <SwipeButton
                  ...
                  colorClass={
                    orderState === "idle" ? "bg-emerald-500" : "bg-blue-600"
                  }
                />
              )}
      ...
      <main className="flex-grow relative h-full bg-slate-100 overflow-hidden">
        {/* Mobile Toggle Button (Open Drawer) */}
        {!isDrawerOpen && (
          <button
            onClick={() => setIsDrawerOpen(true)}
            className="md:hidden absolute top-4 left-4 z-[3000] p-3 bg-white text-slate-800 rounded-xl shadow-lg border border-slate-100 flex items-center gap-2 animate-in fade-in slide-in-from-left-4 duration-300"
          >
            <ICONS.Cart className="w-5 h-5 text-emerald-500" />
        ...
        <MapComponent
          ...
          onProductClick={(id) => {
            if (orderState === "idle") setSelectedProductId(id);
          }}
        ...
        {showNotificationPrompt && (
          <div className="fixed inset-0 z-[5000] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in duration-300">
            <div className="bg-white rounded-3xl shadow-2xl p-8 max-w-sm w-full text-center space-y-6">
              ...
              <div>
                ...
                <p className="text-sm text-slate-500 font-medium">
                  Stay updated with new delivery orders in your area. We'll
                  subscribe you to the "delivery" topic.
                </p>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <button
                  onClick={() => handleNotificationChoice(false)}
                  className="py-3 px-6 rounded-xl border-2 border-slate-100 text-slate-500 font-bold text-sm hover:bg-slate-50 transition-colors"
                >
                  No Thanks
                </button>
                <button
                  onClick={() => handleNotificationChoice(true)}
                  className="py-3 px-6 rounded-xl bg-blue-600 text-white font-bold text-sm hover:bg-blue-700 shadow-lg shadow-blue-600/20 transition-all"
                >
                  Allow
                </button>
              </div>
        ...
        {error && (
          <div className="absolute top-6 left-1/2 -translate-x-1/2 z-[200] bg-slate-900 text-white px-6 py-4 rounded-3xl shadow-2xl flex items-center gap-4 animate-in slide-in-from-top-10 duration-300 border border-white/10">
            ...
            <button
              onClick={() => setError("")}
              className="ml-auto p-2 hover:bg-white/10 rounded-xl"
            >
              <svg
                className="w-4 h-4"
                fill="none"
